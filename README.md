...
- Operator profiles are **local-only**: passcodes are PBKDF2-hashed (210k iterations, Web Crypto) and never leave the browser. They separate training progress between people sharing a machine — they are *not* real authentication; a multi-user deployment needs a backend.
