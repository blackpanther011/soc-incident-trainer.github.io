...
export function buildScenarioPrompt(difficulty, category) {
  ...
  // Noise discipline scales with difficulty: juniors get cleaner logs,
  // senior tiers must dig signal out of realistic operational chatter.
  const noiseGuide = {
    tier1: "Roughly 40% of raw log lines must be routine benign activity (patching, NTP sync, normal user logons, scheduled tasks).",
    tier2: "Roughly 55% of raw log lines must be routine benign activity, including at least one benign event that superficially resembles the attack.",
    tier3: "Roughly 65% of raw log lines must be routine benign activity, including benign admin actions easily mistaken for the attacker.",
    apt:   "Roughly 70% of raw log lines must be routine benign activity; malicious lines must use legitimate tools and blend in (LOLBins, valid accounts).",
  };

  const logSources = {
    ransomware:   "one Windows Security/Sysmon block (Event IDs like 4688, 4624, 7045, 1102 in key=value form) and one firewall or DNS syslog block",
    apt:          "one Windows Security/Sysmon block (4688, 4104, 4624 Type 3, 5861) and one DNS or proxy log block showing beacon periodicity",
    insider:      "one DLP/file-audit block (Event ID 4663, 5145) and one web proxy block (Squid/Zscaler format) showing cloud-storage uploads",
    cloud:        "one AWS CloudTrail JSON block (real eventName/eventSource/userIdentity fields) and one IdP/SSO audit log block",
    supply_chain: "one Windows Sysmon block (process creation + signed-binary network activity) and one EDR/network syslog block",
    web:          "one web server access log block (Apache/Nginx combined format with real-looking URIs and status codes) and one WAF or application syslog block",
  };
...
RAW LOG AUTHENTICITY RULES (critical):
- Logs must use the genuine native format of the source: Windows events as key=value pairs with EventID/TimeCreated/Computer/SubjectUserName fields, syslog as "MMM dd HH:mm:ss host process[pid]: message", proxy logs as timestamped request lines with method/URL/status/bytes/user, CloudTrail as compact JSON.
- ${noiseGuide[difficulty]}
- Interleave malicious lines among benign ones in true chronological order. NEVER label, comment, or visually mark which lines are malicious — finding them IS the exercise.
- Timestamps, hostnames, usernames and IPs must stay perfectly consistent across every section of the scenario (the alert, the raw logs, the IOC list and the narrative must all agree).
- Internal hosts use RFC 1918 addresses; external attacker infrastructure uses plausible public IPs.
...
## 📡 Raw Log Evidence
(Exactly two fenced code blocks: ${logSources[category]}. 8–14 lines each, noise interleaved per the rules above. Title each block with a bold one-line source label, e.g. **Windows Security — FIN-DB-03**.)
...
Keep total under 750 words. Be clinical and precise.`;
}
