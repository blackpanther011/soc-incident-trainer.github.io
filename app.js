...
function renderRadar() {
  const el = $("skillRadar");
  if (!el) return;
  // average score per mode from history
  const agg = {};
  for (const e of state.history) {
    if (e.modeId == null || e.score == null) continue;
    (agg[e.modeId] = agg[e.modeId] || []).push(e.score);
  }
  const data = RADAR_AXES.map((a) => {
    const arr = agg[a.id] || [];
    const avg = arr.length ? arr.reduce((x, y) => x + y, 0) / arr.length : null;
    return { ...a, avg, n: arr.length };
  });
  const played = data.filter((d) => d.avg != null);

  const W = 240, H = 188, cx = W / 2, cy = 96, R = 66;
  ...
  for (let lvl = 1; lvl <= 4; lvl++) {
    const r = (R * lvl) / 4;
    const poly = RADAR_AXES.map((_, i) => pt(i, r).join(",")).join(" ");
  ...
  data.forEach((d, i) => {
    ...
    const dim = d.avg == null ? " dim" : "";
    labels += `<text class="radar-label${dim}" x="${lx.toFixed(1)}" y="${(ly + 3).toFixed(1)}" text-anchor="${anchor}">${d.label}</text>`;
    if (d.avg != null) {
      const [vx, vy] = pt(i, R + 16);
      labels += `<text class="radar-val" x="${vx.toFixed(1)}" y="${(vy + 13).toFixed(1)}" text-anchor="${anchor}">${d.avg.toFixed(1)}</text>`;
    }
  });

  let body;
  if (played.length === 0) {
    body = `${grid}${axes}${labels}<text class="radar-empty" x="${cx}" y="${cy + 3}" text-anchor="middle">No data yet</text>`;
  } else {
    // value polygon — unplayed axes sit at center (0)
    const valPoly = data.map((d, i) => pt(i, R * ((d.avg || 0) / 10)).join(",")).join(" ");
    let dots = "";
    data.forEach((d, i) => { if (d.avg != null) { const [dx, dy] = pt(i, R * (d.avg / 10)); dots += `<circle class="radar-dot" cx="${dx.toFixed(1)}" cy="${dy.toFixed(1)}" r="2.6"/>`; } });
    body = `${grid}${axes}<polygon class="radar-poly" points="${valPoly}"/>${dots}${labels}`;
  }

  let legend = "";
  if (played.length) {
    const best = played.reduce((a, b) => (b.avg > a.avg ? b : a));
    const worst = played.reduce((a, b) => (b.avg < a.avg ? b : a));
    ...
    legend = `<div class="radar-legend">
      <div class="rl-strong"><span class="rl-k">Strongest</span><span class="rl-v">${FULL[best.id]} · ${best.avg.toFixed(1)}</span></div>
      ${played.length > 1 ? `<div class="rl-weak"><span class="rl-k">Focus Area</span><span class="rl-v">${FULL[worst.id]} · ${worst.avg.toFixed(1)}</span></div>` : ""}
    </div>`;
  } else {
    legend = `<div class="radar-legend"><span class="rl-k" style="margin:0 auto">Run incidents in each mode to map your profile</span></div>`;
  }
